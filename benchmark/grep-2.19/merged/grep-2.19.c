...
static size_t __attribute__((__pure__)) compute_bucket_size(size_t candidate,
                Hash_tuning const *tuning) {
float new_candidate;
int tmp;
...
static void *hash_find_entry(Hash_table *table,
                void const *entry,
...
void *hash_delete(Hash_table *table,
                void const *entry) {
void *data;
struct hash_entry *bucket;
...
static _Bool AD_compare(void const *x,
                void const *y) {
...
static size_t AD_hash(void const *x,
                size_t table_size) {
struct Active_dir const *ax;
ax = (struct Active_dir const *)x;
...
__attribute__((__nothrow__)) FTS *(__attribute__((__warn_unused_result__)) fts_open)(char *const *argv,                                                                                      int options,                                                                                      int (*compar)(FTSENT const **,                                                                                                    FTSENT const **));
FTS *(__attribute__((__warn_unused_result__)) fts_open)(char *const *argv,
                int options,
...
void Pcompile(char const *pattern,
                size_t size) {
}
size_t Pexecute(char const *buf,
                size_t size,
                size_t *match_size,
...
void Fcompile(char const *pattern,
                size_t size) {
size_t total;
mb_len_map_t *map;
char const *pat;
...
static void mb_case_map_apply(mb_len_map_t const *map,
                size_t *off,
                size_t *len) {
size_t off_incr;
size_t len_incr;
...
size_t Fexecute(char const *buf,
                size_t size,
                size_t *match_size,
                char const *start_ptr) {
char const *beg;
...
static char tr(char const *trans___0,
                char c) {
unsigned char tmp;
int tmp___0;
if (trans___0) {
...
void kwsincr(kwset_t kwset___1,
                char const *text,
                size_t len) {
struct trie *trie;
...
static void enqueue(struct tree *tree,
                struct trie **last) {
struct trie *tmp;
if (!tree) {
return;
...
static void treefails(struct tree const *tree,
                struct trie const *fail,
...
static void treedelta(struct tree const *tree,
...
static void treenext(struct tree const *tree,
                struct trie **next) {
if (!tree) {
return;
}
...
static _Bool bm_delta2_search(char const **tpp,
                char const *ep,
                char const *sp,
                                              int len,
                char const *trans___0,
...
static char const *memchr_kwset(char const *s,
                size_t n,
                kwset_t kwset___1) {
char const *tmp;
int small_heuristic;
...
static size_t bmexec(kwset_t kwset___1,
                char const *text,
                size_t size) {
...
void GEAcompile(char const *pattern,
                size_t size,
...
size_t EGexecute(char const *buf,
                size_t size,
                size_t *match_size,
                char const *start_ptr) {
...
static size_t mbs_to_wchar(wint_t *pwc,
                char const *s,
                size_t n,
                struct dfa *d) {
unsigned char uc;
...
static _Bool tstbit(unsigned int b,
                charclass_word *const c) {
...
static void copyset(charclass_word *const src,
                charclass_word *dst) {
...
static _Bool equal(charclass_word *const s1,
                charclass_word *const s2) {
int tmp;
tmp = memcmp((void const *)s1, (void const *)s2, sizeof(charclass));
return ((_Bool)(tmp == 0));
}
static void *maybe_realloc(void *ptr,
                size_t nitems,
                size_t *nalloc,
                size_t itemsize) {
void *tmp;
...
static size_t dfa_charclass_index(struct dfa *d,
                charclass_word *const s) {
size_t i;
_Bool tmp;
i = (size_t)0;
while (1) {
...
void dfasyntax(reg_syntax_t bits,
                int fold,
                unsigned char eol) {
unsigned int i;
...
static _Bool setbit_wc(wint_t wc,
                charclass_word *c) {
int b;
int tmp;
tmp = wctob(wc);
...
static int case_folded_counterparts(wchar_t c,
                wchar_t *folded) {
int i;
int n;
wint_t uc;
...
static void copytoks(size_t tindex,
                size_t ntokens) {
size_t i;
if (dfa___0->multibyte) {
i = (size_t)0;
...
static void copy(position_set const *src,
                position_set *dst) {
...
static void merge(position_set const *s1,
                position_set const *s2,
                position_set *m) {
size_t i;
size_t j;
size_t tmp;
...
static state_num state_index(struct dfa *d,
                position_set const *s,
                int context) {
size_t hash;
int constraint;
state_num i;
...
static void epsclosure(position_set *s,
                struct dfa const *d,
                char *visited) {
size_t i;
size_t j;
...
static void build_state(state_num s,
                struct dfa *d) {
state_num *trans___0;
state_num i;
state_num maxstate;
...
ptrdiff_t mb_goback(char const **mb_start,
                char const *cur,
                char const *end) {
char const *p;
char const *p0;
...
wint_t mb_prev_wc(char const *buf,
                char const *cur,
                char const *end) {
char const *p;
ptrdiff_t tmp;
...
wint_t mb_next_wc(char const *cur,
                char const *end) {
wchar_t wc;
mbstate_t mbs;
...
static int file_is_binary(char const *buf,
                size_t bufsize,
                int fd,
                struct stat const *st) {
void *tmp;
...
static int skipped_file(char const *name,
                int command_line,
                int is_dir) {
int tmp___0;
int tmp___2;
...
static int undossify_input(char *buf,
                size_t buflen) {
int chars_left;
return ((int)buflen);
chars_left = 0;
...
static uintmax_t add_count(uintmax_t a,
                uintmax_t b) {
uintmax_t sum;
char *tmp;
sum = a + b;
...
static void print_offset(uintmax_t pos,
                int min_width,
                char const *color) {
...
static void print_line_head(char const *beg,
                char const *lim,
                int sep) {
int pending_sep;
uintmax_t pos;
...
static char const *print_line_middle(char const *beg,
                char const *lim,
...
static void prline(char const *beg,
                char const *lim,
                int sep) {
int matching;
char const *line_color;
...
static _Bool used;
static void prtext(char const *beg,
                char const *lim) {
char eol;
char const *p;
char const *bp;
...
static size_t do_execute(char const *buf,
                size_t size,
                size_t *match_size,
                char const *start_ptr) {
...
static intmax_t grepbuf(char const *beg,
                char const *lim) {
intmax_t outleft0;
char const *p;
char const *endp;
...
static int grepfile(int dirdesc,
                char const *name,
                int follow,
                int command_line) {
int desc;
...
__attribute__((__noreturn__)) void usage(int status);
static void Gcompile(char const *pattern,
                size_t size) {
...
static void Ecompile(char const *pattern,
                size_t size) {
...
static void Acompile(char const *pattern,
                size_t size) {
}
static void GAcompile(char const *pattern,
                size_t size) {
}
static void PAcompile(char const *pattern,
                size_t size) {
}
...
static int prepend_default_options(char const *options,
                int *pargc,
                char ***pargv) {
return (0);
}
...
static int get_nondigit_option(int argc,
                char *const *argv,
...
static _Bool contains_encoding_error(char const *pat,
                size_t patlen) {
mbstate_t mbs;
size_t i;
size_t charlen;
